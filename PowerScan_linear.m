...
start_voltage = 0.25; % Low end of voltage range
end_voltage = 2; % High end of voltage range
dV = 0.0125;
...
for lock_in_freq = [120.588e6]
    for harmonic = [1]
        ...
        file_base = ['PowerSweep_SIGNALGENERATOR_' ...
        ...
        for voltage=[start_voltage:dV:end_voltage end_voltage:-dV:start_voltage]
            ...
            power = round(10*log10((voltage)^2/0.05),1,'decimals');
            if pressed_key == 27
                break;
            end
            ...
            results = [power lock_in.read_real lock_in.read_imaginary lock_in.read_magnitude lock_in.read_frequency];
            data = [data; results];
            ...
            if numel(data == 0)
                line1 = text(0.01,0.98,['Current voltage: ' num2str(voltage) ' V'],'Units','Normalized');
                line2 = text(0.01,0.96,['Current Power: ' num2str(power) ' dBm'],'Units','Normalized');
            else
                line1.String = ['Current Iteration: ' num2str(iteration) ' V'];
